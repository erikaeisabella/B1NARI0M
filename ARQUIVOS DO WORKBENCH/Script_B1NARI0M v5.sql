...
CREATE TABLE IF NOT EXISTS `binariom`.`chamado` (
  ...
  `prioridade` VARCHAR(10) NULL DEFAULT 'Selecionar',
  `localidade` VARCHAR(50) NOT NULL,
  `observacao` VARCHAR(255) NULL,
  `data_abertura` TIMESTAMP CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `binariom`.`chamado_cliente` (
  ...
  INDEX `fk_chamado_cliente_cliente1_idx` (`cliente_matricula` ASC),
  INDEX `fk_chamado_cliente_chamado1_idx` (`chamado_bin` ASC),
  CONSTRAINT `fk_chamado_cliente_chamado1`
    FOREIGN KEY (`chamado_bin`)
    REFERENCES `binariom`.`chamado` (`bin`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_chamado_cliente_cliente1`
